...
By fixed, I meant that the decision regarding the build you will have and the symbols/items to pick after the third payment is usually made for you.
...
The goal here is then to share my learnings and perhaps someone making those games might read this and learn something from it.
...
Engine builder is a big category that contains a lot of sub genres including Deckbuilders and Autobattlers.

Production Engine Builder has 2 characteristics that differentiate it from Deckbuilders.

Firstly, Production Engine Builder's system is automated or almost automated.
...
Deckbuilders and Autobattlers revolve around building an engine to solve different types of problems instead of scaling the output of the engine.
...
I then added a storage area because of this, and made the engine fully configurable without the need to acquire or use any resources; e.g. reroll tokens or removal tokens.
...
For example, in Crop Rotation, you might want to change from 7/3 fruit/vegetable to 7/3 vegetable/fruit due to a new card you acquired.
...
Using Rat and Cat for example, the constraints are distribution constraints at tier 1/2/3 and strict constraints at t4.

Distribution constraints means the efficiency of the card may vary based on the distribution of cards in your deck.
...
A potential solution to this was to add an information fog of war.
The key idea is that cards available to the player are not known at the start of the game until they discover it in draft.
...
Turbo mode comes from some feedback and my own experience that a run might be too long.
I therefore made turbo mode, which is 1 week per season.
...
Secondly, because the Production Curve is tighter and shorter, big pivots will usually cost you the run.
...
In Balatro, you earn interest every $5 and are capped at $25.
...
So the point here is not that a game needs to be short or long, but the length of a run affects the balance of the game (duh).
...
I will probably continue to improve on CR every year since I still enjoy working and playing it.

If you are a player of CR, I hope you learn something regarding why CR is the way it is.
If you are a dev and want to make an Engine Builder, I hope you have learnt something from this.
